				...
				<ul className={`list-reset ${styles.MapContent__items}`}>
					{cities.map((city) => (
						<li className={styles.MapContent__item}>
							<h4 className={styles.MapContent__item_title}>{city.data.title}</h4>
							<address>{city.data.address}</address>
							<span className={styles.MapContent__time}>{city.data.schedule}</span>
							<a href={`tel:${city.data.phoneNumber.replace(/[^0-9]/g,'')}`}>{city.data.phoneNumber}</a>
							<button className="btn-reset">
								...
								<span onClick={()=> onItemClick(city.coords)}>Проложить маршрут</span>
							</button>
						</li>
					))}
